...
- **No planning-doc references in committed code.** Comments, log lines, identifiers, docstrings, AND committed-doc/README files must not reference internal planning artifacts. This covers (non-exhaustively): phase numbers (`V0.1`, `V1.5`, `Phase 2`), task numbers (`Task 6`, `V0.3 Task 9`), forward-looking process language (`will be implemented in V0.3`, `V2 hook`, `not yet wired up in V0.1`), and **any filename matching `*_PLAN.md`, `*_ISSUES.md`, `*_INCONSISTENCIES.md`, `*_HYGIENE_PLAN.md`, or anything inside `agent-sdk-planning/`** — every one of those is gitignored personal workspace by convention. References rot fast, leak internal process, and are meaningless to future readers. Before adding any `// See FOO.md § X` comment, verify `FOO.md` is actually tracked in git (`git ls-files FOO.md` returns a result); if it doesn't, write the explanation inline instead of pointing at a doc the reader can't open. Describe what the code does today and the non-obvious WHY — not the development process that produced it.
